        ...
        <div class="p-6 flex justify-between items-center text-gray-900">
          <div class="w-full relative overflow-x-auto shadow-md sm:rounded-lg">
            <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
              ...
              <tbody>
                ...
                <tr
                  ...
                  <th scope="row"
                    class="px-6 py-4 font-medium text-gray-900 whitespace-nowrap dark:text-white text-center">
                    {{ $review->client_company_name }}
                  </th>
                  <td class="px-6 py-4 border text-center w-[150px]">
                    @if ($review->status === 'active')
                    <p class="bg-green-600 py-2 w-[90px] rounded-md text-white">{{ $review->status }}</p>
                    @else
                    <p class="bg-yellow-600 py-2 w-[90px] rounded-md text-white ">{{ $review->status }}</p>
                    @endif
                  </td>
            ...
            <nav class="flex items-center justify-between pt-4 px-3" aria-label="Table navigation">
              <span class="text-sm font-normal text-gray-500 dark:text-gray-400">{{ $reviews->links() }}</span>
            </nav>
          </div>
        </div>
      </div>
